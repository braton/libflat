...
int main(void) {
	...
	FOR_ROOT_POINTER(&stringset_root.rb_node,
		FOR_POINTER(const struct rb_node*,node,&stringset_root.rb_node,
			FLATTEN_STRUCT(string_node,(const struct string_node*)node);
		);
	);
